...
-- Select * from dataset where brand name is either Adiadas or Puma but
-- the discount value should be between 1000 and 1500 


SELECT * FROM PRODUCTS
WHERE
(BRAND_NAME = 'Adidas' OR BRAND_NAME = 'Puma') AND
(discounted_price between 1000 and 1500);
...
-- Find all data not of brand H&M and Roadster having rating > 3 and marked_price greater than 500 
-- only of category tshirts


SELECT * FROM products
WHERE  brand_name NOT IN ('H&M','Roadster') AND rating > 3 AND marked_price > 500
