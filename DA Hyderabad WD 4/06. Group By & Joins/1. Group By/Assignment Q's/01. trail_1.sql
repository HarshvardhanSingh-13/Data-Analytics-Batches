...
select * from products order by marked_price desc limit 1;

# 2. Finding the least expensive product on myntra
select * from products order by marked_price asc limit 1;

# 3. Finding the top 5 most expensive product on myntra
select * from products order by marked_price desc limit 5;
...
where product_tag = 'tshirts' 
	and (brand_tag = 'nike' or brand_tag = 'adidas') 
		and discounted_price between 1000 and 1200
        
order by discounted_price asc,brand_name desc;