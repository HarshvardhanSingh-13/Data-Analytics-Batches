...
SELECT brand_name,discounted_price,rating,rating_count FROM products;
...
SELECT brand_name,rating,rating_count, (discounted_price * rating_count) AS "revenue"
FROM products;


SELECT brand_name,rating,rating_count,ROUND(((marked_price-discounted_price)/marked_price)*100,0) as "discount %"
FROM products;
...
SELECT * FROM products WHERE rating > 3;
...
SELECT distinct(product_name),rating,rating_count,brand_name FROM products WHERE brand_name = "Adidas";
...
SELECT DISTINCT(product_name), brand_tag FROM products WHERE brand_tag = 'adidas' and discounted_price >5000;
...
FROM products
WHERE brand_name = 'Adidas' and discounted_price > 5000 and marked_price > 8000;
...
-- Q) Find Product Tag of 'Roadster' with discounted price between 3000 and 8000 and rating > 3 
--    and rating count > 12

SELECT DISTINCT(product_tag), brand_name,rating,rating_count,discounted_price FROM products
WHERE brand_name = "Roadster" and discounted_price BETWEEN 3000 and 5000 and rating > 3 and rating_count > 12;
...
-- Q) FIND unique products of Brand Adidas or Puma where discounted price between 3000 and 8000


SELECT DISTINCT(product_name),brand_name,discounted_price,rating
FROM products
WHERE (brand_name = "Adidas" or brand_name = "Puma") and discounted_price BETWEEN 3000 and 8000;
...
SELECT DISTINCT(product_name),brand_name,discounted_price,rating FROM products
WHERE brand_name NOT IN ("H&M","Biba") and discounted_price BETWEEN 2000 and 4000;
